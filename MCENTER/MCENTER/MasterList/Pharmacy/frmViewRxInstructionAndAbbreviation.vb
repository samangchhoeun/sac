...
Public Class frmViewRxInstructionAndAbbreviation
    ...
    Private Sub frmViewRxInstructionAndAbbreviation_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
        Dispose()
    End Sub
    ...
    Public Sub LoadDataToList(Optional ByVal Find_ID As Integer = 0, Optional ByVal _Flag As Integer = 0)
        Try
            dtData = GetDataFromDBToTable("SA_GetInstructionAbbreviationByID", New SqlParameter("@ID", Find_ID), New SqlParameter("@isList", _Flag))
    ...
    Private Sub frmViewRxInstructionAndAbbreviation_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LoadDataToList()
    End Sub
    ...
    Public Sub GetSearchDataByID(Find_ID As Integer, Optional ByVal _Flag As Integer = 0)
        Try
            Dim dtData As DataTable = GetDataFromDBToTable("SA_GetInstructionAbbreviationByID", New SqlParameter("@ID", Find_ID), New SqlParameter("@isList", _Flag))
            With dtData
                If .Rows.Count = 1 Then
                    ...
                    txtAbb.Text = .Rows(0)("AbbKey").ToString
                    txtEn.Text = .Rows(0)("FullWordEn").ToString
                    txtKh.Text = .Rows(0)("FullWordKh").ToString
    ...
    Private Sub bbiPRemove_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiPRemove.ItemClick
        Try
            With gvData
                ...
                detected = XtraMessageBox.Show("Do you want to remove the selected abbreviation word?", "Confirm Remove?", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
    ...
    Private Sub DisabledItems(Optional _IDTempList As String = "", Optional _Inactive As Integer = 0)
        Try
            OpenCon(Con)
            Dim cmd As New SqlCommand("SA_DisabledInstructionAbbreviation", Con)
    ...
    Private Sub bbiPRestore_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiPRestore.ItemClick
        Try
            With gvData
                ...
                detected = XtraMessageBox.Show("Do you want to restore the selected abbreviation word?", "Confirm Remove?", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                If detected = DialogResult.No Then Return
    ...
    Private Sub bbiPExport_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiPExport.ItemClick
        ExportToXlsx(dtData, "Rx Instruction and Abbreviation Report")
    End Sub

    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        If String.IsNullOrEmpty(txtAbb.Text.Trim) OrElse String.IsNullOrEmpty(txtEn.Text.Trim) Then
            XtraMessageBox.Show("Please enter abbreviation word.", "Error Help?", MessageBoxButtons.OK, MessageBoxIcon.Error)
            txtAbb.Focus()
            Return
        End If
        ...
        Try
            OpenCon(Con)
            Dim cmd As New SqlCommand("SA_SaveInstructionAbbreviation", Con)
            cmd.CommandType = CommandType.StoredProcedure
            With cmd.Parameters
                .AddWithValue("@ID", _TempID)
                .AddWithValue("@AbbKey", txtAbb.Text.Trim)
            ...
            LoadDataToList()
            XtraMessageBox.Show(cmd.Parameters("@Msg").Value.ToString, "Add Abbreviation Word", MessageBoxButtons.OK, MMS)
            btnNew.Focus()
        Catch ex As Exception
            XtraMessageBox.Show(ex.Message, "Error Help?", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
