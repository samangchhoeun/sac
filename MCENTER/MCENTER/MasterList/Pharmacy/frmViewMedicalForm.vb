...
Public Class frmViewMedicalForm
    ...
    Private Sub frmViewMedicalForm_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
        InitPageRefresh()
        Dispose()
    End Sub
    ...
    Public Sub LoadDataToList(Optional ByVal Find_ID As Integer = 0, Optional ByVal _Flag As Integer = 0)
        Try
            dtData = GetDataFromDBToTable("SA_GetMedicalFormByID", New SqlParameter("@ID", Find_ID), New SqlParameter("@isList", _Flag))
    ...
    Private Sub frmViewMedicalForm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LoadDataToList()
    End Sub
    ...
    Public Sub GetSearchDataByID(Find_ID As Integer, Optional ByVal _Flag As Integer = 0)
        Try
            Dim dtData As DataTable = GetDataFromDBToTable("SA_GetMedicalFormByID", New SqlParameter("@ID", Find_ID), New SqlParameter("@isList", _Flag))
            With dtData
                If .Rows.Count = 1 Then
                    ...
                    txtEn.Text = .Rows(0)("FormEn").ToString
                    txtKh.Text = .Rows(0)("FormKh").ToString
    ...
    Private Sub bbiPRemove_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiPRemove.ItemClick
        Try
            With gvData
                ...
                detected = XtraMessageBox.Show("Do you want to remove the selected medical form?", "Confirm Remove?", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
    ...
    Private Sub DisabledItems(Optional _IDTempList As String = "", Optional _Inactive As Integer = 0)
        Try
            OpenCon(Con)
            Dim cmd As New SqlCommand("SA_DisabledMedicalForm", Con)
    ...
    Private Sub bbiPRestore_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiPRestore.ItemClick
        Try
            With gvData
                ...
                detected = XtraMessageBox.Show("Do you want to restore the selected medical form?", "Confirm Remove?", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                If detected = DialogResult.No Then Return
                Dim _ID As Integer = CInt(.GetRowCellValue(.FocusedRowHandle, "TranID").ToString)
    ...
    Private Sub bbiPExport_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiPExport.ItemClick
        ExportToXlsx(dtData, "Medical Form Report")
    End Sub

    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        If String.IsNullOrEmpty(txtEn.Text.Trim) Then
            XtraMessageBox.Show("Please enter medical form.", "Error Help?", MessageBoxButtons.OK, MessageBoxIcon.Error)
            txtEn.Focus()
            Return
        End If
        ...
        Try
            OpenCon(Con)
            Dim cmd As New SqlCommand("SA_SaveMedicalForm", Con)
            cmd.CommandType = CommandType.StoredProcedure
            With cmd.Parameters
                .AddWithValue("@ID", _TempID)
                .AddWithValue("@FormEn", txtEn.Text.Trim)
                .AddWithValue("@FormKh", txtKh.Text.Trim)
            ...
            LoadDataToList()
            XtraMessageBox.Show(cmd.Parameters("@Msg").Value.ToString, "Add Medical Form", MessageBoxButtons.OK, MMS)
            btnNew.Focus()
        Catch ex As Exception
            XtraMessageBox.Show(ex.Message, "Error Help?", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
