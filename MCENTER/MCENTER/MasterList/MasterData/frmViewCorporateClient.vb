...
Public Class frmViewCorporateClient
    Dim _ID As Integer = 0
    Dim dtData As New DataTable

    Private Sub frmViewCorporateClient_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
        Dispose()
    End Sub
    ...
    Private Sub frmViewCorporateClient_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LoadDataToList()
    End Sub

    Public Sub LoadDataToList(Optional ByVal _ID As Integer = 0, Optional ByVal _Flag As Integer = 0)
        Try
            dtData = GetDataFromDBToTable("SA_GetCorporateClientsByID", New SqlParameter("@ID", _ID), New SqlParameter("@isList", _Flag))
    ...
    Private Sub bbiPModify_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiPModify.ItemClick
        ...
        '        GetSearchDataByID(CInt(SelectedRow("ID")))
        '    End If
        'Catch ex As Exception
        '    XtraMessageBox.Show("Oops, something went wrong. Please try again", "Error Help?", MessageBoxButtons.OK, MessageBoxIcon.Information)
        'End Try
    ...
    Public Sub GetSearchDataByID(Optional Find_ID As Integer = 0, Optional _Flag As Integer = 0)
        Try
            Dim dtData As DataTable = GetDataFromDBToTable("SA_GetCorporateClientsByID", New SqlParameter("@ID", Find_ID), New SqlParameter("@isList", _Flag))
            With dtData
                If .Rows.Count = 1 Then
                    EnabledEdit()
                    EnabledButtonSave("&Update", True, True)
                    btnNew.Enabled = True
                    txtClientCode.Text = .Rows(0)("ID").ToString
                    _ID = CInt(.Rows(0)("ID"))
                    txtClient.Text = .Rows(0)("CorporateClient").ToString
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        If String.IsNullOrEmpty(txtClient.Text.Trim) Then
        ...
        Try
            OpenCon(Con)
            Dim cmd As New SqlCommand("SA_SaveCorporateClient", Con)
            cmd.CommandType = CommandType.StoredProcedure
            With cmd.Parameters
                .AddWithValue("@ID", _ID)
                .AddWithValue("@Client", txtClient.Text.Trim)
            ...
            LoadDataToList()
            XtraMessageBox.Show(cmd.Parameters("@Msg").Value.ToString, "Add Corporate Client", MessageBoxButtons.OK, MMS)
            btnNew.Focus()
        Catch ex As Exception
    ...
    Private Sub bbiPRemove_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiPRemove.ItemClick
        Try
            With gvData
                ...
                detected = XtraMessageBox.Show("Do you want to remove the selected corporate clients?", "Confirm Remove?", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
    ...
    Private Sub DisabledItems(Optional _IDTempList As String = "", Optional _Inactive As Integer = 0)
        Try
            OpenCon(Con)
            Dim cmd As New SqlCommand("SA_DisabledClients", Con)
            cmd.CommandType = CommandType.StoredProcedure
            With cmd.Parameters
    ...
    Private Sub bbiPRestore_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiPRestore.ItemClick
        Try
            With gvData
                ...
                detected = XtraMessageBox.Show("Do you want to restore the selected corporate clients?", "Confirm Remove?", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                If detected = DialogResult.No Then Return
                Dim _ID As Integer = CInt(.GetRowCellValue(.FocusedRowHandle, "ID").ToString)
    ...
    Private Sub bbiPExport_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiPExport.ItemClick
        ExportToXlsx(dtData, "Corporate Clients Report")
    End Sub
End Class