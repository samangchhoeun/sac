...
Public Class frmViewDiagnosisGroup
    Dim _ID As Integer = 0
    Dim dtData As New DataTable

    Private Sub frmViewDiagnosisGroup_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
        Dispose()
    End Sub
    ...
    Private Sub frmViewDiagnosisGroup_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LoadDataToList()
    End Sub

    Public Sub LoadDataToList(Optional ByVal _ID As Integer = 0, Optional ByVal _Flag As Integer = 0)
        Try
            dtData = GetDataFromDBToTable("SA_GetDiagnosisGroupByID", New SqlParameter("@ID", _ID), New SqlParameter("@isList", _Flag))
    ...
    Private Sub bbiPModify_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiPModify.ItemClick
        Try
            With gvData
                If .RowCount <= 0 Then
                    ...
                    Return
                End If
                Dim _ID As Integer = CInt(.GetRowCellValue(.FocusedRowHandle, "Code").ToString)
        ...
        '        GetSearchDataByID(CInt(SelectedRow("Code")))
        '    End If
        'Catch ex As Exception
        '    XtraMessageBox.Show("Oops, something went wrong. Please try again", "Error Help?", MessageBoxButtons.OK, MessageBoxIcon.Information)
        'End Try
    ...
    Public Sub GetSearchDataByID(Optional Find_ID As Integer = 0, Optional _Flag As Integer = 0)
        Try
            Dim dtData As DataTable = GetDataFromDBToTable("SA_GetDiagnosisGroupByID", New SqlParameter("@ID", Find_ID), New SqlParameter("@isList", _Flag))
            With dtData
                If .Rows.Count = 1 Then
                    EnabledEdit()
                    EnabledButtonSave("&Update", True, True)
                    btnNew.Enabled = True
                    txtCode.Text = .Rows(0)("Code").ToString
                    _ID = CInt(.Rows(0)("Code"))
                    txtDxGroupEn.Text = .Rows(0)("DxGroupEn").ToString
                    txtDxGroupKh.Text = .Rows(0)("DxGroupKh").ToString
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        If String.IsNullOrEmpty(txtDxGroupEn.Text.Trim) Then
            XtraMessageBox.Show("Please enter diagnosis group name.", "Error Help?", MessageBoxButtons.OK, MessageBoxIcon.Error)
            txtDxGroupEn.Focus()
            Return
        End If
        ...
        Try
            OpenCon(Con)
            Dim cmd As New SqlCommand("SA_SaveDiagnosisGroup", Con)
            cmd.CommandType = CommandType.StoredProcedure
            With cmd.Parameters
                .AddWithValue("@ID", _ID)
                .AddWithValue("@DxGroupEn", txtDxGroupEn.Text.Trim)
                .AddWithValue("@DxGroupKn", txtDxGroupKh.Text.Trim)
            ...
            LoadDataToList()
            XtraMessageBox.Show(cmd.Parameters("@Msg").Value.ToString, "Add Diagnosis Group", MessageBoxButtons.OK, MMS)
            btnNew.Focus()
        Catch ex As Exception
    ...
    Private Sub bbiPRemove_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiPRemove.ItemClick
        Try
            With gvData
                ...
                detected = XtraMessageBox.Show("Do you want to remove the selected diagnosis groups?", "Confirm Remove?", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
    ...
    Private Sub DisabledItems(Optional _IDTempList As String = "", Optional _Inactive As Integer = 0)
        Try
            OpenCon(Con)
            Dim cmd As New SqlCommand("SA_DisabledDiagnosisGroup", Con)
            cmd.CommandType = CommandType.StoredProcedure
            With cmd.Parameters
        ...
        Try
            With gvData
                If .RowCount <= 0 Then
                    ...
                    Return
                End If
                detected = XtraMessageBox.Show("Do you want to restore the selected diagnosis groups?", "Confirm Remove?", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                If detected = DialogResult.No Then Return
                Dim _ID As Integer = CInt(.GetRowCellValue(.FocusedRowHandle, "Code").ToString)
    ...
    Private Sub bbiPExport_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiPExport.ItemClick
        ExportToXlsx(dtData, "Diagnosis Group Report")
    End Sub
End Class