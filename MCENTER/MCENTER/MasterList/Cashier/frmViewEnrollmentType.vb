...
Public Class frmViewEnrollmentType
    ...
    Private Sub frmViewEnrollmentType_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
        Dispose()
    End Sub
    ...
    Public Sub LoadDataToList(Optional ByVal Find_ID As Integer = 0, Optional ByVal _Flag As Integer = 0)
        Try
            dtData = GetDataFromDBToTable("SA_GetEnrollmentTypeByID", New SqlParameter("@ID", Find_ID), New SqlParameter("@isList", _Flag))
    ...
    Private Sub frmViewEnrollmentType_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LoadDataToList()
    End Sub
    ...
    Public Sub GetSearchDataByID(Find_ID As Integer, Optional ByVal _Flag As Integer = 0)
        Try
            Dim dtData As DataTable = GetDataFromDBToTable("SA_GetEnrollmentTypeByID", New SqlParameter("@ID", Find_ID), New SqlParameter("@isList", _Flag))
            With dtData
                If .Rows.Count = 1 Then
                    ...
                    txtMembershipType.Text = .Rows(0)("EnrollmentType").ToString
    ...
    Private Sub bbiPRemove_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiPRemove.ItemClick
        Try
            With gvData
                ...
                detected = XtraMessageBox.Show("Do you want to remove the selected enrollment type?", "Confirm Remove?", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
    ...
    Private Sub DisabledItems(Optional _IDTempList As String = "", Optional _Inactive As Integer = 0)
        Try
            OpenCon(Con)
            Dim cmd As New SqlCommand("SA_DisabledEnrollmentType", Con)
        ...
        Try
            With gvData
                If .RowCount <= 0 Then
                    ...
                    Return
                End If
                detected = XtraMessageBox.Show("Do you want to restore the selected enrollment type?", "Confirm Remove?", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                If detected = DialogResult.No Then Return
                Dim _ID As Integer = CInt(.GetRowCellValue(.FocusedRowHandle, "TranID").ToString)
    ...
    Private Sub bbiPExport_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles bbiPExport.ItemClick
        ExportToXlsx(dtData, "Enrollment Type Report")
    End Sub

    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        If String.IsNullOrEmpty(txtMembershipType.Text.Trim) Then
            XtraMessageBox.Show("Please enter enrollment type.", "Error Help?", MessageBoxButtons.OK, MessageBoxIcon.Error)
            txtMembershipType.Focus()
            Return
        End If
        ...
        Try
            OpenCon(Con)
            Dim cmd As New SqlCommand("SA_SaveEnrollmentType", Con)
            cmd.CommandType = CommandType.StoredProcedure
            With cmd.Parameters
                .AddWithValue("@ID", _TempID)
                .AddWithValue("@EnrollmentType", txtMembershipType.Text.Trim)
            ...
            LoadDataToList()
            frmViewMembershipFee.LoadEnrollmentType()
            XtraMessageBox.Show(cmd.Parameters("@Msg").Value.ToString, "Add Enrollment Type", MessageBoxButtons.OK, MMS)
            btnNew.Focus()
        Catch ex As Exception
            XtraMessageBox.Show(ex.Message, "Error Help?", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
